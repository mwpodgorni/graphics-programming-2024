...
void ParticlesApplication::Update(const Window& window, float deltaTime)
{
	...
	if (window.IsMouseButtonPressed(Window::MouseButton::Left))
	{
		...
		glm::vec2 velocity = 0.5f * (mousePosition - m_mousePosition) / deltaTime;
...
void ParticlesApplication::Render(float deltaTime)
{
	// Clear background
	...
	m_shaderProgram.SetUniform(m_currentTimeUniform, deltaTime);
...
{
	std::cout << "EmitParticle" << std::endl;
