...
GearsApplication::GearsApplication() : Application(1024, 1024, "Gears demo"),
m_colorUniform(-1),
...
void GearsApplication::Render()
{
	...
	m_shaderProgram.SetUniform(m_projectionUniform, m_camera.GetViewProjectionMatrix());
	...
	glm::mat4 rightGearMatrix(glm::translate(glm::vec3(0.75f, 0.0f, 0.0f)) * glm::rotate(-16 / 8 * speed * time, axis));
	...
	glm::mat4 yellowGearMatrix(glm::translate(glm::vec3(0.0f, -0.2f, 0.0f)) * glm::rotate(-speed * time, axis));

	DrawGear(m_smallGear, centerGearMatrix * yellowGearMatrix, Color(1.0f, 1.0f, 0.0f));
...
void GearsApplication::InitializeShaders()
{
	...
	m_projectionUniform = m_shaderProgram.GetUniformLocation("ProjectionMatrix");
